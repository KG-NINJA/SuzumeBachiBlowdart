...
## Prediction pipeline checks

For pipeline safety changes use
`python -m unittest discover -s tests -p 'test_pipeline_safety.py' -v`
from `.github/workflows/pipeline_safety_tests.yml`. For tuning changes use
`python -m pytest tests/test_xgboost_tuning.py` from
`.github/workflows/xgboost_safeguards.yml`.
Read only the relevant `README.md` topic and affected prediction/training
workflow. Preserve data/financial controls and inactive workflow state;
instructions alone require no training, prediction or dashboard deployment.
Keep changes localized, understand a failure before refactoring and explain its
cause with the scoped diff. Follow the current request rather than assuming
every task concerns failing tests in `./src`.
