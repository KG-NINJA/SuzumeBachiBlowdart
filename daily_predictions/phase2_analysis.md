# 📈 Phase 2: Market Environment Analysis

**Generated:** 2025-11-28T08:28:01.985160

## 🎯 Market Regime

| Metric | Value |
|--------|-------|
| **Regime** | STRONG_DOWNTREND 🔴 SELL |
| **Bullish** | 0/10 (0.0%) |
| **Bearish** | 9/10 (90.0%) |
| **Hold** | 0/10 (10.0%) |

## 📊 Interpretation

Market is strongly bearish (90.0%). Focus on SELL signals. BUY signals should be avoided.

---

## 🏆 Relative Strength Analysis

| Rank | Ticker | Accuracy | Strength | Signal |
|------|--------|----------|----------|--------|
| 1 | AMD | 98.25% | 🟡 NEUTRAL | +0.14 |
| 2 | AAPL | 99.05% | 🟡 NEUTRAL | +0.10 |
| 3 | NVDA | 98.38% | 🟡 NEUTRAL | +0.08 |
| 4 | GOOGL | 99.59% | 🟡 NEUTRAL | +0.04 |
| 5 | MSFT | 98.67% | 🟡 NEUTRAL | +0.02 |


---

## 🎲 Adjusted Trading Recommendations

### 🟢 EXECUTE
- **NVDA** 📉 SELL | Conf: 69.15% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **AAPL** 📉 SELL | Conf: 71.24% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **MSFT** 📉 SELL | Conf: 62.19% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **GOOGL** 📉 SELL | Conf: 63.91% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **AMZN** 📉 SELL | Conf: 50.74% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **META** 📉 SELL | Conf: 57.99% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **TSLA** 📉 SELL | Conf: 59.71% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **AMD** 📉 SELL | Conf: 76.57% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)
- **NFLX** 📉 SELL | Conf: 45.44% | Strength: NEUTRAL
  → ✅ EXECUTE - SELL (STRONG_DOWNTREND environment, NEUTRAL strength)


### ⚠️ CAUTION
- **QQQ** - ⚠️ CAUTION - BUY signal but STRONG_DOWNTREND environment - wait for confirmation


### ⏸️ SKIP
All predictions are actionable.


---

## 🎯 Trading Strategy by Market Regime

### Current: STRONG_DOWNTREND

**Strategy:**
• Focus on SELL signals
• Avoid BUY signals
• Use tight stops
• Scale into shorts

---

*Phase 2 Report - Market Environment Detection*
*Next: Phase 3 - Backtesting & Performance Tracking*
